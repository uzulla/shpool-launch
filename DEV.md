...
- 絞り込み結果が0件の Enter は新規セッション名として query を返す。空白を含む query は作成しない。query が `-` で始まり cwd 既定項目がある場合は `defaultItem + query` を返し、cwd 既定項目がない場合は作成しない。返す名前は `session.Sanitize` で cwd 由来名と同じ文字集合 (`[A-Za-z0-9._-]`、それ以外は `_`) に正規化する。
