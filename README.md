...
- 絞り込み結果が0件の状態で Enter すると、入力した文字列で新規セッションを作成する。空白を含む入力は作成しない。cwd 候補がある状態で `-another` のように `-` から入力した場合は、cwd 候補にサフィックスとして足して `work.company-a.api-another` のように作成する。cwd 候補が無い状態で `-scratch` のように `-` から入力した場合は、オプションと誤認されうるため作成しない。
